"""Wait for dataset v4, launch kernel v4 once, then recover and validate it."""
...
def main():
    try:
        marker = WORK / "launch-v4-attempt.json"
        if marker.exists():
            raise RuntimeError("Kernel v4 launch already attempted")
        write(status="WAITING_FOR_DATASET_V4", monitor_status="ACTIVE", last_error=None)
        ...
        while time.monotonic() < deadline:
            ...
            if "KernelWorkerStatus.ERROR" in status:
                error_dir = WORK / "error-output-v4"
                error_dir.mkdir(exist_ok=True)
                run([KAGGLE, "kernels", "output", KERNEL, "-p", error_dir, "--quiet"], 900)
                raise RuntimeError("Remote kernel v4 error: " + status)
            ...
            raise TimeoutError("Kernel v4 timeout")
        result = WORK / "result-v4"

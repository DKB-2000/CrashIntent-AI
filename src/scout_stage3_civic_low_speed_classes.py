"""Sensor-only class-balance scout on the remaining disjoint Chunk3 routes."""
import io
import json
import os
import subprocess
...
OUT=P/'artifacts/stage3-civic-low-speed-class-scout-20260916'
...
def prepare():
    ...
    plan=dict(status='RESERVED_SENSOR_ONLY',routes=routes,segments=selected,
              ...
              rules='speed2-5,angle/pose/other-segment-debiased-gyro consensus; centered11 persistence; same thresholds as failed calibration, no retuning',
              ...
              scope='Only6 remaining disjoint Chunk3 recording routes. No video, model predictions or held evaluation signals.')
...
def run():
    ...
    try:
        ...
        for i,seg in enumerate(plan['segments']):
            data[seg]=signals(z,seg,plan['offset_deg'])
        ...
        for route in plan['routes']:
            ...
            for target in segs:
                ...
                assert mask.sum()>=30
                bias=float(np.median(a['gyro'][mask]-a['yaw'][mask]))
                high=b['valid']&(b['speed']>=5)&(b['speed']<=35)
                mae=float(np.mean(np.abs(b['gyro'][high]-bias-b['yaw'][high]))) if high.any() else None
                speed=b['speed'];angle=b['angle'];yaw=b['yaw'];gyro=b['gyro']-bias
                domain=b['valid']&(speed>=2)&(speed<5)&(np.abs(yaw)<.12)
                ...
                rows.append(dict(route=route,segment=target,low_valid=int(domain.sum()),high_bias_transfer_mae=mae,
        ...
        for route in plan['routes']:
            ...
            summary.append(dict(route=route,segments=2,low_valid=sum(r['low_valid'] for r in group),classes=counts,
                                high_mae_max=max(r['high_bias_transfer_mae'] for r in group)))
        summary.sort(key=lambda r:(-sum(v>=30 for v in r['classes'].values()),-r['low_valid'],r['route']))
        result=dict(status='COMPLETE_VALIDATED',segments=rows,routes=summary,transfer_bytes=remote.received,
                    class_balanced_routes=[r for r in summary if all(v>=30 for v in r['classes'].values()) and r['high_mae_max']<=.005],
                    validation='Fixed scout reservation and thresholds; other-segment gyro bias; finite ordered source sensors; no model or held-route values used.')

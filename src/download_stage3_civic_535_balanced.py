...
ROOT = PROJECT/'data_raw/comma2k19/civic-535-balanced-20260916'
STATE = PROJECT/'artifacts/stage3-civic-535-balanced-acquisition-20260916'
...
def prepare():
    ...
    chosen=max(scout['class_balanced_routes'],key=lambda r:(min(r['classes'].values()),sum(r['classes'].values())))
    chosen_routes=[chosen['route']]
    split={chosen['route']:'evaluation'}
    selected=sorted(s for s in scout_plan['segments'] if s.rsplit('/',1)[0]==chosen['route'])
    assert len(selected)==2 and min(chosen['classes'].values())>=20
    ...
    with zipfile.ZipFile(remote) as archive:
        ...
        for i,segment in enumerate(selected):
            ...
            for name in required+[n for n in optional if prefix+n in entries]:
                ...
                files.append(dict(ID=f'CIVIC_535_{i:03}',segment=segment,member=entry.filename,
        ...
        save(STATE/'plan.json',dict(source=url,revision=revision,archive=metadata,seed=SEED,
                                   ...
                                   frozen_class_counts=chosen['classes'],minimum_rows=90,minimum_per_class=20,
                                   use='Best minimum-class-support route selected by frozen sensor-only 5-35m/s scout before video acquisition or model predictions.',
...
def run():
    ...
    for i,segment in enumerate(plan['segments']):
        sid=f'CIVIC_535_{i:03}';folder=ROOT/sid

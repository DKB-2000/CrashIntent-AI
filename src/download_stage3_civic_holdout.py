...
ROOT = PROJECT/'data_raw/comma2k19/civic-holdout-20260914'
STATE = PROJECT/'artifacts/stage3-civic-holdout-acquisition-20260914'
...
def prepare():
    ...
    with zipfile.ZipFile(remote) as archive:
        ...
        prior_path=PROJECT/'artifacts/stage3-civic-acquisition-20260914/plan.json'
        previous_routes={r.split('/')[-1] for r in json.loads(prior_path.read_text())['routes']}
        eligible=[s for s in eligible if s.split('/')[-2] not in previous_routes]
        ...
        for i,segment in enumerate(selected):
            ...
            for name in required+[n for n in optional if prefix+n in entries]:
                ...
                files.append(dict(ID=f'CIVIC_HOLDOUT_{i:03}',segment=segment,member=entry.filename,
        ...
        save(STATE/'plan.json',dict(source=url,revision=revision,archive=metadata,seed=SEED,
                                   ...
                                   use='Reserved new recording routes, excluding all previous Civic routes. No training, predictions or calibration tuning.',
                                   limits='1GiB/run,60min worker,3 attempts/range,one worker,OS lock',
                                   scope='Same Civic device as pilot, new recording route IDs; same highway dataset, not geographic independence. Partial ZIP CRC32 and local SHA256; full archive SHA256 not verified.'))
...
def run():
    ...
    for i,segment in enumerate(plan['segments']):
        sid=f'CIVIC_HOLDOUT_{i:03}';folder=ROOT/sid

"""Launch once, monitor, recover and validate the Imperial feature CV trial."""
import hashlib,json,os,re,subprocess,time,zipfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; WORK=ROOT/'artifacts/kaggle-stage1-imperial-feature-cv-20260918'; KAGGLE=ROOT/'.venv/Scripts/kaggle.exe'
KERNEL='biadis/crashintent-stage1-imperial-feature-cv'
def write(**values):
...
def main():
 try:
  ...
  result=WORK/'result'; run([KAGGLE,'kernels','output',KERNEL,'-p',result,'--file-pattern','stage1-imperial-feature-cv-result.zip','--quiet'],900)
  files=list(result.rglob('stage1-imperial-feature-cv-result.zip'))
  ...
  if report.get('status')!='COMPLETE' or len(predictions)!=300: raise ValueError('Coverage validation failed')
  keys={row['image'] for row in predictions}
  if len(keys)!=100: raise ValueError('Prediction identity validation failed')
  ...
  write(status='COMPLETE',monitor_status='VALIDATED',completed_predictions=300,result_sha256=hashlib.sha256(files[0].read_bytes()).hexdigest(),last_error=None)

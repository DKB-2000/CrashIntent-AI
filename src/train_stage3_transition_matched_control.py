...
ROOT=P/'artifacts/stage3-transition-matched-control-20260914'
...
def run():
    ...
    assert len(new)==1090 and set(new.route).isdisjoint(valid.route)
    base.ROOT=REF
    ...
    xnew=np.empty((1090,xold.shape[1]),np.float32);xnew[:1000]=xold
    ...
    rows=[];events=[];outputs={}
    for seed in plan['seeds']:
        for arm in ['original']+plan['arms']:
            folder=REF/f'{seed}-motion' if arm=='original' else ROOT/arm/str(seed)
    ...
    result=dict(status='COMPLETE_VALIDATED',decision='FOLLOWUP_ONLY' if all(g['passed'] for g in gates.values()) else 'KEEP_BASELINE',
        averages={a:float(np.mean([r['f1'] for r in pooled if r['arm']==a])) for a in ['original']+plan['arms']},gates=gates,metrics=rows,transitions=transition,
        validation='Input/manifest hashes; new feature anchors exact; frozen normalization exact; all9 checkpoint/hash/logit replays exact; new6 CSV mapping exact; independent confusion metrics; transitions and frozen gate evaluated.',

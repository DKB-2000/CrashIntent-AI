...
ROOT=P/'artifacts/stage3-civic-535-balanced-evaluation-20260917'
ACQ=P/'artifacts/stage3-civic-535-balanced-acquisition-20260916'
RAW=P/'data_raw/comma2k19/civic-535-balanced-20260916'
...
def prepare():
 ...
 old=json.loads((P/'artifacts/stage3-frozen-civic-holdout-20260916/plan.json').read_text(encoding='utf-8-sig'))
 ap=json.loads((ACQ/'plan.json').read_text(encoding='utf-8'));av=ACQ/'validation.json';assert json.loads(av.read_text())['status']=='PASS'
 plan=dict(status='FROZEN_BEFORE_LABELS',evaluation_routes=ap['routes'],offset_deg=old['offset_deg'],rules=old['rules'],models=old['models'],seeds=old['seeds'],model_sha256=old['model_sha256'],minimum_rows=ap['minimum_rows'],minimum_per_class=ap['minimum_per_class'],gate='Each seed and the frozen route F1 improve over original; no class recall falls >2%p. Diagnostic FOLLOWUP only, never auto-adopt.',acquisition_validation_sha256=sha(av),script_sha256=sha(__file__),scope='Sensor-stratified single Civic route frozen before video/model scoring. Proxy labels, not human or official truth.')
 save('plan.json',plan);save('status.json',dict(status='PREPARED',completed=0,total=4));print(json.dumps(plan,indent=2))
def run():
 ...
 evaluation={r for r,s in ap['route_splits'].items() if s=='evaluation'};assert len(evaluation)==1;assert set(plan['evaluation_routes'])==evaluation
 frames=[];videos=[]
 for i,seg in enumerate(ap['segments']):
  route=seg.rsplit('/',1)[0]
  if route not in evaluation:continue
  sid=f'CIVIC_535_{i:03}';folder=RAW/sid;frames.append(labels(sid,folder,route,plan['offset_deg']))
...
def watch():
 ...
 with (ROOT/'watch.lock').open('a+b') as lock:
  lock.write(b'0');lock.flush();lock.seek(0);msvcrt.locking(lock.fileno(),msvcrt.LK_NBLCK,1);save('execution.json',dict(status='STARTED',pid=os.getpid()))
  try:

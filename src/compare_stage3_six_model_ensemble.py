"""Frozen equal-logit ensemble of warmstart_ce and right_weighted seeds."""
import hashlib,json
...
ROOT=P/'artifacts/stage3-six-model-ensemble-20260917'
CE=P/'artifacts/stage3-prediction-preservation-20260914/warmstart_ce'
RIGHT=P/'artifacts/stage3-right-weighted-control-20260916/right_weighted'
...
def civic_logits(models):
 ...
 for path in sorted(CIVIC.glob('*-features.npz')):
  ...
  parts.append(pd.DataFrame(dict(ID=sid,sample_index=np.arange(len(f)),single=np.argmax(np.mean(logits[:3],axis=0),1),ensemble=np.argmax(np.mean(logits,axis=0),1))))
...
def run():
 ...
 paths=[CE/str(s) for s in SEEDS]+[RIGHT/str(s) for s in SEEDS]
 source={str((f/'model.pt').relative_to(P)):sha(f/'model.pt') for f in paths}
 plan=dict(status='FROZEN',seeds=SEEDS,families=['warmstart_ce','right_weighted'],combination='Unweighted arithmetic mean of six raw accel and steer logits; no temperatures, weights or thresholds.',baseline='Equal-logit ensemble of three warmstart_ce seeds',gate='Old validation: both heads improve, >=3/4 steer routes improve, no class recall loss >2pp, transition misses/delay noninferior. Civic: steer F1 improves and no recall loss >2pp.',source_sha256=source,script_sha256=sha(__file__))
 ...
 for folder in paths:
  assert sha(folder/'model.pt')==source[str((folder/'model.pt').relative_to(P))]
 ...
 single_a=np.mean(acc[:3],axis=0).argmax(1);single_s=np.mean(steer[:3],axis=0).argmax(1);ens_a=np.mean(acc,axis=0).argmax(1);ens_s=np.mean(steer,axis=0).argmax(1);moving=valid.accel.to_numpy()!=3
 ...
 save('report.json',result);save('status.json',dict(status='COMPLETE_VALIDATED',completed=6,total=6,decision=result['decision']));print(json.dumps(result,indent=2))

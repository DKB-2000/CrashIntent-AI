...
BUNDLE = ROOT / 'artifacts/kaggle-stage2-precision-20260911'
KERNEL = 'biadis/crashintent-stage2-precision-check'
        ...
        if 'KernelWorkerStatus.COMPLETE' in state or 'KernelWorkerStatus.ERROR' in state:
            ...
            for attempt in range(5):
                run([KAGGLE, 'kernels', 'output', KERNEL, '-p', result_dir,
                     '--file-pattern', 'stage2-precision-result.zip', '--quiet'], timeout=900)
                files = list(result_dir.rglob('stage2-precision-result.zip'))
                if len(files) == 1:
                    break
                if attempt == 4:
            ...
            output = run([sys.executable, ROOT / 'src/validate_stage2_precision.py',

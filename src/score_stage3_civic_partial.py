...
ROOT=P/'artifacts/stage3-civic-partial-score-20260914'
LABELS=P/'artifacts/stage3-civic-partial-labels-20260914'
...
def run():
    ...
    labels=pd.read_csv(labelpath);assert labels.partial_valid_steer.sum()==714 and len(labels)==2400
    ...
    acquisition=json.loads((P/'artifacts/stage3-civic-acquisition-20260914/validation.json').read_text())
    save('plan.json',dict(model_zip_sha256=sha(archive),checkpoint_sha256=sha(ck),label_sha256=sha(labelpath),
         ...
         scoring='Fixed714 sensor-consensus points; predictions on all2400 points; fixed3class macroF1; excluded70.25%; no tuning'))
    results=[];videos=[]
    for i,(sid,g) in enumerate(labels.groupby('ID')):
        save('status.json',dict(status='SCORING',completed=i,total=4,pid=os.getpid()))
        ...
        print(f'Validated prediction {i+1}/4 {sid}',flush=True)
    pred=pd.concat(results,ignore_index=True)
    joined=labels.merge(pred,on=['ID','sample_index'],validate='one_to_one');assert len(joined)==2400
    ...
    report=dict(status='COMPLETE_VALIDATED',classes=CLASSES,pooled=pooled,
          ...
          total=2400,scored=714,excluded=1686,excluded_fraction=.7025,
          ...
          scope='Selected stable sensor-consensus subset,4videos2routes/21episodes, same highway different vehicle. Not human truth, full-domain or official performance. CPU only.')
    save('report.json',report);save('status.json',dict(status='COMPLETE_VALIDATED',completed=4,total=4,scored=714))

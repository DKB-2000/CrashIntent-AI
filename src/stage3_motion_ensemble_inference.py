"""Standalone Stage3 motion-only deployment using a fixed three-seed ensemble."""
...
_S3M_FORMAT='stage3-motion-ensemble-v1'
...
def s3_motion_load(path,device):
    ...
    if not isinstance(states,list) or len(states)!=3:raise ValueError('Expected three ensemble models')
    models=[]
    for state in states:
        model=S3MotionModel();model.load_state_dict(state,strict=True);models.append(model.to(device).eval())
    ...
    try:
        ...
        with torch.inference_mode():
            for start in range(0,len(features),256):
                ...
                logits_per_model=[member(torch.from_numpy(x).to(device)) for member in model]
                logits=tuple(torch.stack([value[k] for value in logits_per_model]).mean(0) for k in range(2))

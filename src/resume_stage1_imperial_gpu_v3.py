"""Wait for dataset v3, launch kernel v3 once, then recover and validate it."""
...
def main():
    try:
        marker = WORK / "launch-v3-attempt.json"
        if marker.exists():
            raise RuntimeError("Kernel v3 launch already attempted")
        write(status="WAITING_FOR_DATASET_V3", monitor_status="ACTIVE", last_error=None)
        ...
        while time.monotonic() < deadline:
            ...
            if "KernelWorkerStatus.ERROR" in status:
                error_dir = WORK / "error-output-v3"
                error_dir.mkdir(exist_ok=True)
                run([KAGGLE, "kernels", "output", KERNEL, "-p", error_dir, "--quiet"], 900)
                raise RuntimeError("Remote kernel v3 error: " + status)
            ...
            raise TimeoutError("Kernel v3 timeout")
        result = WORK / "result-v3"

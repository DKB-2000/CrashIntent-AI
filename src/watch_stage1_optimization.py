...
BUNDLE = ROOT / 'artifacts/kaggle-stage1-optimization-20260910'
KERNEL = 'biadis/crashintent-stage1-optimization-check'
        ...
        if 'KernelWorkerStatus.COMPLETE' in state or 'KernelWorkerStatus.ERROR' in state:
            ...
            for attempt in range(5):
                run([KAGGLE, 'kernels', 'output', KERNEL, '-p', result_dir,
                     '--file-pattern', 'stage1-optimization-result.zip', '--quiet'], timeout=900)
                files = list(result_dir.rglob('stage1-optimization-result.zip'))
                if len(files) == 1:
                    break
                if attempt == 4:
            ...
            output = run([sys.executable, ROOT / 'src/validate_stage1_optimization.py',

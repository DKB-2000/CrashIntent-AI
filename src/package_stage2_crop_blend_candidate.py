...
OUT=ROOT/'artifacts/stage2-crop-blend-submit-candidate-20260917'
...
def predict_stage2(data_dir,model_dir):
    ...
    if checkpoint.get('crop_format')!='stage2-semantic-crop-blend-v2' or checkpoint.get('crop_seeds')!=[20260825,20260826,20260827,20260916,20260917] or checkpoint.get('task_weight')!=.5:raise ValueError('Stage2 crop blend checkpoint contract mismatch')
    ...
    del backbone,temporal,probes,task_evasion,task_side;torch.cuda.empty_cache()
...
def main():
 ...
 ev=torch.tensor([int(r['evasion_space']) for r in rows]);side=torch.tensor([int(r['entry_side']=='RIGHT') for r in rows]);states=[];task_ev_states=[];task_side_states=[];train_predictions=[]
 for seed in SEEDS:
  ...
  probe.eval();states.append(probe.state_dict());shared_logits=probe(xn).detach()
  evx=torch.cat((xn[:,:768],xn[:,1280:1792],xn[:,2304:2816]),1);sx=torch.cat((xn[:,:768],xn[:,768:1280],xn[:,1792:2304]),1)
  torch.manual_seed(seed);ev_probe=nn.Linear(1792,2);side_probe=nn.Linear(1792,2);task_opt=torch.optim.AdamW(list(ev_probe.parameters())+list(side_probe.parameters()),lr=1e-3,weight_decay=.1)
  for _ in range(EPOCHS):
   ev_logits=ev_probe(evx);side_logits=side_probe(sx);loss=nn.functional.cross_entropy(ev_logits,ev)+nn.functional.cross_entropy(side_logits,side);task_opt.zero_grad(set_to_none=True);loss.backward();task_opt.step()
  ev_probe.eval();side_probe.eval();task_ev_states.append(ev_probe.state_dict());task_side_states.append(side_probe.state_dict());task_logits=torch.cat((ev_probe(evx),side_probe(sx)),1).detach();train_predictions.append(.5*shared_logits+.5*task_logits)
 ...
 checkpoint={'model':temporal.state_dict(),'labels':p.OUTPUT_COLUMNS,'crop_format':'stage2-semantic-crop-blend-v2','crop_seeds':list(SEEDS),'crop_epochs':EPOCHS,'task_weight':.5,'crop_mean':mean,'crop_std':std,'crop_probes':states,'task_evasion_probes':task_ev_states,'task_side_probes':task_side_states}

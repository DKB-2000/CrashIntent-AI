"""Paired route-held-out motion vs short controls on the fixed training sample pool."""
...
ROOT=Path('artifacts/stage3-route-cv-20260911');SEEDS=[20260910,20260911,20260912];ARMS=['baseline','short']
...
def train():
 ...
 f=pd.read_csv(ROOT/'samples.csv');x={arm:source.arrays(f,arm) for arm in ARMS};results=[];started=time.monotonic();mask=torch.ones(1682);mask[:1280]=0
 for fold in range(5):
  ...
  for seed in SEEDS:
   for arm in ARMS:
    out=ROOT/f'{fold}-{seed}-{arm}';out.mkdir();mean=x[arm][ti].mean(0);std=x[arm][ti].std(0).clip(.01);xt=torch.from_numpy(np.clip((x[arm][ti]-mean)/std,-10,10))*mask;xv=torch.from_numpy(np.clip((x[arm][vi]-mean)/std,-10,10))*mask
    ...
    report=dict(fold=fold,seed=seed,arm=arm,train_rows=len(ti),validation_rows=len(vi),metrics=score(valid,a.argmax(1),s.argmax(1)),checkpoint_sha256=sha(out/'model.pt'),history=history)
...
def verify():
 ...
 x={a:source.arrays(f,a) for a in ARMS};runs=json.loads((ROOT/'results.json').read_text())['runs'];assert len(runs)==30
 ...
 for r in runs:
  ...
  mean=x[arm][ti].mean(0);std=x[arm][ti].std(0).clip(.01);np.testing.assert_array_equal(ck['mean'],mean);np.testing.assert_array_equal(ck['std'],std);np.testing.assert_array_equal(ck['mask'],np.r_[np.zeros(1280),np.ones(402)])
 ...
 for fold in range(5):
  b=np.load(ROOT/f'fold-{fold}-batches.npy');tr=f[f.fold!=fold];assert b.shape==(1200,10) and np.all(tr.group.to_numpy()[b]==np.arange(10))
 summary=[];routes=[]

"""Combine the validated Stage2 candidate with fixed-seed short-window Stage3; preserve Stage1/2 bytes."""
import ast,hashlib,importlib.util,json,shutil,zipfile
from pathlib import Path
import numpy as np,pandas as pd,torch
from daily_submission import inspect_zip

OUT=Path('artifacts/final-stage2-short-stage3-20260911')
GPU=Path('artifacts/kaggle-final-short-candidate-20260911')
SOURCE=Path('artifacts/stage3-temporal-motion-20260910')
BASE=Path('artifacts/stage2-pretrained-submit-candidate-20260911/submit.zip')
MODULE=Path('src/stage3_short_motion_inference.py')
...
def main():
 ...
 assert not (OUT/'validation.json').exists(), 'Refuse overwrite of validated candidate'
 baseline_validation=json.loads((BASE.parent/'validation.json').read_text(encoding='utf-8-sig'))
 assert baseline_validation['status']=='STATIC_AND_CPU_PARITY_PASS'
 assert sha(BASE)==baseline_validation['sha256']
 assert baseline_validation['stage2_checkpoint_sha256']==sha(BASE.parent/'best.pt')
 ...
 source=SOURCE/'20260910-short';report=json.loads((source/'report.json').read_text())
 assert sha(source/'model.pt')==report['checkpoint_sha256']
 assert sha(BASE)==baseline_validation['sha256']
 ckpt=torch.load(source/'model.pt',map_location='cpu',weights_only=True)
 ckpt.update(format='stage3-motion-short-control-v1',accel_classes=['ACCELERATING','DECELERATING','CONSTANT','STOPPED'],steer_classes=['LEFT','STRAIGHT','RIGHT'])
 ...
 for sid in ['COMMA2K19_C1_0022','COMMA2K19_C1_0075']:
  ...
  with np.load(SOURCE/'cache'/(sid+'.npz')) as z:np.testing.assert_array_equal(actual,z['short'][:,1280:])
 ...
 result=dict(status='CPU_PASS_GPU_PENDING',candidate_sha256=digest,bytes=path.stat().st_size,
  ...
  code_sha256=sha(MODULE),stage2_validation_sha256=sha(BASE.parent/'validation.json'),stage2_checkpoint_sha256=baseline_validation['stage2_checkpoint_sha256'],stage3_windows=[3,5],submitted=False)

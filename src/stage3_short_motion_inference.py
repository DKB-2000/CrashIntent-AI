"""Standalone Stage3 short-window deployment of the fixed 20260910 control."""
...
_S3M_FORMAT='stage3-motion-short-control-v1'
...
def s3_motion_features(path):
    ...
    previous=None;history=deque(maxlen=5);rows=[]
    try:
        while True:
            ok,bgr=cap.read()
            if not ok:break
            ...
            rows.append(np.r_[values[-1],np.mean(values[-3:],axis=0),np.mean(values,axis=0)])
            previous=gray
    finally:cap.release()
...
def s3_motion_load(path,device):
    ...
    if (checkpoint['mode']!='short' or checkpoint.get('windows')!=[3,5]) or mask.shape!=(1682,) or torch.any(mask[:1280]!=0) or torch.any(mask[1280:]!=1):

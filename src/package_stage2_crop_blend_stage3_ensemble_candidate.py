"""Combine validated Stage2 crop blend with Stage3 warmstart_ce ensemble."""
...
OUT=P/'artifacts/stage2-crop-blend-stage3-ensemble-submit-candidate-20260917'
BASE=P/'artifacts/stage2-crop-blend-submit-candidate-20260917/submit.zip'
...
BASE_SHA='86e6d6562ad784520415e66682e1d7ac4e90d0c5cb1b729364361351534f5bee'
...
def main():
 ...
 with zipfile.ZipFile(BASE) as src:
  ...
  combined_tree=ast.parse(inference)
  before={n.name:ast.dump(n,include_attributes=False) for n in tree.body if isinstance(n,(ast.FunctionDef,ast.ClassDef)) and n.name in ('predict_stage1','predict_stage2')}
  after={n.name:ast.dump(n,include_attributes=False) for n in combined_tree.body if isinstance(n,(ast.FunctionDef,ast.ClassDef)) and n.name in before}
  assert before==after and set(before)=={'predict_stage1','predict_stage2'}
 ...
 digest=inspect_zip(target);report=dict(status='STATIC_AND_CPU_PARITY_PASS_GPU_PENDING',candidate_sha256=digest,bytes=target.stat().st_size,base_sha256=sha(BASE),stage2_checkpoint_sha256=sha(P/'artifacts/stage2-crop-blend-submit-candidate-20260917/best.pt'),deploy_checkpoint_sha256=sha(best),source_checkpoint_sha256={str(s):sha(SOURCE/str(s)/'model.pt') for s in SEEDS},changed_entries=changed,selection='Validated Stage2 crop blend plus fixed equal-logit ensemble of warmstart_ce seeds 20260910/11/12',cpu_parity_rows=checked,max_logits_error=maxerr,classification_matches=True,stage1_stage2_requirements_unchanged=True,stage1_stage2_ast_preserved=True,zip_crc_pass=True,gpu_check='Not run',submitted=False)

"""Watch an already-approved Stage3 expanded run; download and validate its output only."""
import datetime
import json
import os
...
BUNDLE = ROOT / 'artifacts/kaggle-stage3-candidate-20260910'
KERNEL = 'biadis/crashintent-stage3-candidate-check'
    ...
    while time.monotonic() - started < 10800:
        ...
        if 'KernelWorkerStatus.COMPLETE' in state or 'KernelWorkerStatus.ERROR' in state:
            ...
            for attempt in range(5):
                run([KAGGLE, 'kernels', 'output', KERNEL + '/1', '-p', result_dir,
                     '--file-pattern', 'candidate-integration-result.zip', '--quiet'], timeout=900)
            ...
            output = run([sys.executable, ROOT / 'src/validate_candidate_gpu_result.py', '--result-zip', files[0], '--candidate', ROOT/'artifacts/stage3-submit-candidate-20260910/submit.zip', '--fixtures', BUNDLE/'dataset/public-fixtures.bin', '--output', BUNDLE/'result-validation.json'], timeout=600)
            (BUNDLE / 'validation.log').write_text(output, encoding='utf-8')
            validation = json.loads((BUNDLE / 'result-validation.json').read_text())

"""Launch once, monitor, recover and validate the VDMoire trial."""
import hashlib,json,os,re,subprocess,time,zipfile
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; WORK=ROOT/'artifacts/kaggle-stage1-vdmoire-20260918'; KAGGLE=ROOT/'.venv/Scripts/kaggle.exe'
KERNEL='biadis/crashintent-stage1-vdmoire-diagnostic'; DATASET='biadis/crashintent-stage1-vdmoire-clips'
def write(**values):
...
def main():
 try:
  ...
  result=WORK/'result'; run([KAGGLE,'kernels','output',KERNEL,'-p',result,'--file-pattern','stage1-vdmoire-result.zip','--quiet'],900)
  files=list(result.rglob('stage1-vdmoire-result.zip'))
  ...
  if report.get('status')!='COMPLETE' or len(predictions)!=16: raise ValueError('Coverage validation failed')
  keys={(row['model'],row['video']) for row in predictions}
  if len(keys)!=16: raise ValueError('Prediction identity validation failed')
  if any(not 0<=row['probability']<=1 or row['answer']!=('RERECORDED' if row['probability']>=.5 else 'ORIGINAL') for row in predictions): raise ValueError('Prediction value validation failed')
  validated=WORK/'validated'; validated.mkdir(exist_ok=True); (validated/'report.json').write_text(json.dumps(report,indent=2)); (validated/'predictions.json').write_text(json.dumps(predictions,indent=2))
  write(status='COMPLETE',monitor_status='VALIDATED',completed_predictions=16,result_sha256=hashlib.sha256(files[0].read_bytes()).hexdigest(),last_error=None)

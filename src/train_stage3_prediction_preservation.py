...
def run():
    ...
    xv=norm(xvalid);xs={'warmstart_ce':norm(xnew),'warmstart_preserve':norm(xnew)}
    frames={'warmstart_ce':new,'warmstart_preserve':new};completed=0
        ...
        for arm in plan['arms']:
            ...
            for epoch,g in schedule.groupby('epoch',sort=True):
                ...
                for _,batch in g.groupby('batch',sort=True):
                    ...
                    penalty=torch.zeros(())
                    if arm=='warmstart_preserve' and anchor.any():
                ...
                history.append(dict(epoch=int(epoch),loss=float(np.mean(losses)),updates=len(losses),preservation_penalty=float(np.mean(penalties))))
    ...
    result=dict(status='COMPLETE_VALIDATED',decision='FOLLOWUP_ONLY' if any(g['passed'] for g in gates.values()) else 'KEEP_BASELINE',
        averages={a:float(np.mean([r['f1'] for r in pooled if r['arm']==a])) for a in ['original','straight_only']+plan['arms']},gates=gates,preservation_effect=preservation_effect,metrics=rows,transitions=transition,
        validation='Input/manifest/features hashes; all6 initial checkpoints exact; frozen normalization exact; all12 checkpoint/hash/logit replays exact; new6 CSV mapping exact; independent confusion metrics; transitions and frozen gate evaluated.',

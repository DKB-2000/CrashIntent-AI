...
ROOT = PROJECT/'data_raw/comma2k19/civic-535-training-20260917'
STATE = PROJECT/'artifacts/stage3-civic-535-training-acquisition-20260917'
...
def prepare():
    ...
    scout_root=PROJECT/'artifacts/stage3-civic-5-35-class-scout-20260916'
    ...
    assert scout['status']=='COMPLETE_VALIDATED' and len(scout['class_balanced_routes'])>=1
    ...
    # Freeze the route with the largest minimum class support before model predictions.
    evaluation=max(scout['class_balanced_routes'],key=lambda r:(min(r['classes'].values()),sum(r['classes'].values())))
    chosen=[r for r in scout['class_balanced_routes'] if r['route']!=evaluation['route']]
    chosen_routes=[r['route'] for r in chosen]
    split={r:'training' for r in chosen_routes}
    selected=sorted(s for s in scout_plan['segments'] if s.rsplit('/',1)[0] in split)
    assert len(selected)==6 and len(chosen_routes)==3 and all(min(r['classes'].values())>=20 for r in chosen)
    ...
    with zipfile.ZipFile(remote) as archive:
        ...
        for i,segment in enumerate(selected):
            ...
            for name in required+[n for n in optional if prefix+n in entries]:
                ...
                files.append(dict(ID=f'CIVIC_TRAIN_{i:03}',segment=segment,member=entry.filename,
        ...
        save(STATE/'plan.json',dict(source=url,revision=revision,archive=metadata,seed=SEED,
                                   ...
                                   frozen_class_counts={k:sum(r['classes'][k] for r in chosen) for k in ('LEFT','STRAIGHT','RIGHT')},minimum_rows=90,minimum_per_class=20,
                                   held_out_evaluation_route=evaluation['route'],
                                   use='Three remaining class-balanced routes reserved for training; best-balanced route remains disjoint evaluation.',
...
def run():
    ...
    for i,segment in enumerate(plan['segments']):
        sid=f'CIVIC_TRAIN_{i:03}';folder=ROOT/sid

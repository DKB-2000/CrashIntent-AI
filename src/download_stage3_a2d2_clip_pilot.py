...
OUT = Path("artifacts/stage3-a2d2-clip-pilot-20260917")
RAW = Path("data_raw/a2d2/clip-pilot-20180810_150607")
BASE = (
    "https://audi-autonomous-driving-dataset.s3.eu-central-1.amazonaws.com/"
    "camera_lidar/20180810_150607/camera/cam_front_center"
)
PREFIX = "20180810150607_camera_frontcenter_"
...
def download_one(index: int, deadline: float) -> dict:
    ...
    for attempt in range(1, 4):
        try:
            ...
            return {"index":index, "file":str(destination), "bytes":destination.stat().st_size, "sha256":file_hash(destination)}
        except Exception:
            if attempt == 3:
                raise
...
def write_clip(indices: list[int], destination: Path) -> dict:
    destination.parent.mkdir(parents=True, exist_ok=True)
    ...
    try:
        for index in indices:
            image = cv2.imread(str(RAW / frame_name(index)))
            if image is None:
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError("A2D2 clip pilot already running") from exc
    ...
    try:
        ...
        for number,row in enumerate(plan["selections"]):
            ...
            clips.append({"id":f"A2D2_PILOT_{number:03d}", "label":row["label"], "center":center, "indices":indices})
        unique=sorted({i for clip in clips for i in clip["indices"]})
        completed=[]
        mutex=threading.Lock()
        save(status_path,{"status":"DOWNLOADING", "completed":0, "total":len(unique), "clips":len(clips), "pid":os.getpid()})
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            futures={executor.submit(download_one,index,deadline):index for index in unique}
        ...
        for number,clip in enumerate(clips,1):
            video=write_clip(clip["indices"], OUT/"videos"/f"{clip['id']}.mp4")
            clip_reports.append({**clip, **video})
            sample_rows.append({"ID":clip["id"], "route":"A2D2_20180810_150607", "endpoint":15, "steer_label":clip["label"]})
        ...
        report={
            ...
            "images":sorted(completed,key=lambda x:x["index"]), "clip_records":clip_reports,
        }

...
BUNDLE = ROOT / 'artifacts/kaggle-stage3-continuation-20260910'
KERNEL = 'biadis/crashintent-stage3-continuation'
        ...
        if 'KernelWorkerStatus.COMPLETE' in state or 'KernelWorkerStatus.ERROR' in state:
            ...
            for attempt in range(5):
                run([KAGGLE, 'kernels', 'output', KERNEL + '/1', '-p', result_dir,
                     '--file-pattern', 'stage3-route-trial-result.zip', '--quiet'], timeout=900)
                files = list(result_dir.rglob('stage3-route-trial-result.zip'))
                if len(files) == 1:
                    break
                if attempt == 4:
            ...
            output = run([sys.executable, ROOT / 'src/validate_stage3_route_trial.py', '--root', BUNDLE], timeout=600)

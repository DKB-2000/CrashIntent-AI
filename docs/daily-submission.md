...
Stage1 본학습 모델의 자동 통합검사·일일 후보 등록 후속 작업은
[Stage1 자동 ZIP 연결](stage1-auto-release.md)에 정리했다.
2026-09-10 현재 코드·설정 준비 완료, 새 원격 전송 범위에 대한 자동 승인 검토 차단으로
후속 예약은 아직 미등록이다. 기존 일일08:30 작업은 계속 기존 selected.json을 사용한다.

2026-09-10 16:30 KST 사용자 승인 후 Stage1 후속 예약 등록·첫 실행 성공(LastTaskResult=0).
앞 절의 미등록 상태는 해소됐다. 2분 간격으로 선행 검증 완료를 확인해 통합검사 후
새 ZIP을 즉시 생성·등록하며, 기존08:30 작업은 이후 등록된 후보를 복사한다.
